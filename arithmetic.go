...
func (x *Decimal) Sub(y *Decimal) *Decimal {
	...
	for i := len(yd) - 1; i > k; i-- {
		if xd[i] < yd[i] {
			for j := i; j > 0 && xd[j-1] == 0; {
				j--
				xd[j] = BASE - 1
			}
			xd[i-1]--
			// Fix: need to find first non-zero
			xd[i] += BASE
		}
		xd[i] -= yd[i]
	}
...
func (x *Decimal) Mul(y *Decimal) *Decimal {
	...
	if xd == nil || len(xd) == 0 || xd[0] == 0 ||
		yd == nil || len(yd) == 0 || yd[0] == 0 {

		r := &Decimal{ctx: ctx}

		if sign == 0 || (xd != nil && xd[0] == 0 && (yd == nil || len(yd) == 0)) ||
			(yd != nil && yd[0] == 0 && (xd == nil || len(xd) == 0)) {
			// NaN
			r.s = 0
			r.d = nil
			r.e = 0
...
func (x *Decimal) Div(y *Decimal) *Decimal {
	return divide(x, x.newFromDecimal(y), 0, RoundingMode(0), false, 0)
}
