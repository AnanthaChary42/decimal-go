...
func divide(x, y *Decimal, pr int, rm RoundingMode, dp bool, base int) *Decimal {
	...
	if xd == nil || len(xd) == 0 || yd == nil || len(yd) == 0 ||
		(xd != nil && xd[0] == 0) || (yd != nil && yd[0] == 0) {

		q := &Decimal{ctx: ctx}

		if x.s == 0 || y.s == 0 ||
			(xd != nil && yd != nil && xd[0] == yd[0] && len(xd) == 1 && len(yd) == 1) ||
			(xd == nil && yd == nil) {
			// NaN
			q.s = 0
			q.d = nil
	...
	for {
		...
		if cmp < 0 {
			...
			if k > 1 {
				...
				if cmp == 1 {
					...
					if yL < prodL {
						divSubtract(prod, yz, prodL, base32)
					} else {
						divSubtract(prod, yd, prodL, base32)
					}
					prod = prod // update reference
				}
			} else {
				if k == 0 {
					cmp = 1
			...
			divSubtract(rem, prod, remL, base32)
			...
			if cmp == -1 {
				...
				if cmp < 1 {
					k++
					if yL < remL {
						divSubtract(rem, yz, remL, base32)
					} else {
						divSubtract(rem, yd, remL, base32)
					}
				}
		...
		xi++
		sd--
		if !(xi <= xL || (len(rem) > 0 && rem[0] != 0)) || sd <= 0 {
...
func divSubtract(a, b []int32, aL int, base int32) {
	...
	for aL2 := aL - 1; aL2 >= 0; aL2-- {
		...
		a[aL2] = i*base + a[aL2] - bv
	}

	// Remove leading zeros.
	for len(a) > 1 && a[0] == 0 {
		a = a[1:]
		// Note: this doesn't modify the caller's slice header.
